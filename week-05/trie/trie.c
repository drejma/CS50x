...
// Saves popular dog names in a trie (https://www.dailypaws.com/dogs-puppies/dog-names/common-dog-names)
...
bool check(char* word)
{
    ...
    {
        // Calculate supposed index for a char of a word
        index = tolower(word[i]) - 'a';

        // Check if that index exists in a node
        if (cursor->children[index] == NULL)
        {
            return false;
        }

        // If it exist, move cursor to that child
        cursor = cursor->children[index];
    }

    // If all word chars exist in a trie, check if they complete a word
    if (cursor->is_word)
    {
        return true;
    }
